...
class Smsir
{
    ...
    public static function credit()
    {
        ...
        $result = $client->get(
            ...
            [
                'headers' => ['x-sms-ir-secure-token' => self::getToken()],
                'connect_timeout' => 30,
                'idn_conversion' => false,
            ]
        );
    ...
    public static function getReceivedMessages($perPage, $pageNumber, $formDate, $toDate)
    {
        $client = new Client();
        $result = $client->get(
            "http://restfulsms.com/api/ReceiveMessage?Shamsi_FromDate={$formDate}&Shamsi_ToDate={$toDate}&RowsPerPage={$perPage}&RequestedPageNumber={$pageNumber}",
            [
                'headers' => ['x-sms-ir-secure-token' => self::getToken()],
                'connect_timeout' => 30,
                'idn_conversion' => false,
    ...
    public static function getSentMessages($perPage, $pageNumber, $formDate, $toDate)
    {
        $client = new Client();
        $result = $client->get(
            "http://restfulsms.com/api/MessageSend?Shamsi_FromDate={$formDate}&Shamsi_ToDate={$toDate}&RowsPerPage={$perPage}&RequestedPageNumber={$pageNumber}",
            [
                'headers' => ['x-sms-ir-secure-token' => self::getToken()],
                'connect_timeout' => 30,
                'idn_conversion' => false,
